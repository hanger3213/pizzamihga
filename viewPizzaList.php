...
<body>
    ...
    <div class="container my-3" id="cont">
        <div class="col-lg-4 text-center bg-light my-3" style="margin:auto;border-top: 2px groove black;border-bottom: 2px groove black;">     
            <h2 class="text-center"><span id="catTitle">Детали</span></h2>
        </div>
        <div class="row">
        <?php
            $id = $_GET['catid'];
                                ...
                                if($loggedin){
                                    ...
                                    if($quaExistRows == 0) {
                                        echo '<form action="partials/_manageCart.php" method="POST">
                                              ...
                                              <button type="submit" name="addToCart" class="btn btn-primary mx-2">Добавить в корзину</button>';
                                    }else {
                                        echo '<a href="viewCart.php"><button class="btn btn-primary mx-2">В корзину</button></a>';
                                    }
                                }
                                else{
                                    echo '<button class="btn btn-primary mx-2" data-toggle="modal" data-target="#loginModal">Добавить в корзину</button>';
                                }
                            echo '</form>                            
                                <a href="viewPizza.php?pizzaid=' . $pizzaId . '" class="mx-2"><button class="btn btn-primary">Быстрый просмотр</button></a> 
                                </div>
                            </div>
                        </div>
                    </div>';
            ...
            if($noResult) {
                echo '<div class="jumbotron jumbotron-fluid">
                    <div class="container">
                        <p class="display-4">Извините. В этой категории нет доступных товаров.</p>
                        <p class="lead"> Попробуйте обновить позже.</p>
                    </div>
                </div> ';
            }
            ?>
